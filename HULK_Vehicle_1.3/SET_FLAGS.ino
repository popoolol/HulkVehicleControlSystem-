...
// system_bool = [PRINTING, MOVING] [LIMIT HIT, START UP] [MANUAL, AUTOMATIC] [WHEEL, STABILIZER] 
...
// Flag if a stablizer reached a limit
// Can be any stabilizer, up or down
void set_limited_flag()
{
...
void set_delimited_flag()
{
    if  (bitRead(system_bool, 5)) {system_bool -=   B100000;}
    if (!bitRead(system_bool, 4)) {system_bool +=    B10000;}
    lcd.clear();
    lcd.print("STBLZR(S) MOVED ");
}
