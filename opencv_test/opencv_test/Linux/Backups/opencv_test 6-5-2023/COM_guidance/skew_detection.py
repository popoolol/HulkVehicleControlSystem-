...
def get_camera_skew(centers_and_depths):
    """
    Calculate the skew of the camera in degrees based on the detected templates.
    """
    # Calculate the difference in depth and x-coordinate between the two templates
    dx = 0.36195

    #ensures that the templates are calculated as right and left properly
    if (centers_and_depths[0][0] < centers_and_depths[1][0]):
        ...
        dz = 0    
    skew_angle = np.arctan2(dx, dz) # Note that in camera coordinates, z axis is depth.
...
try:
    while True:
        ...
        for i in indices:
            ...
            # Store the center and depth for skew calculation
        ...
        # After the for loop, calculate the skew of the camera
        if len(centers_and_depths) >= 2:  # We need at least two templates to calculate the skew
            skew_angle = get_camera_skew(centers_and_depths)
            print(f"Camera skew: {skew_angle} degrees")
