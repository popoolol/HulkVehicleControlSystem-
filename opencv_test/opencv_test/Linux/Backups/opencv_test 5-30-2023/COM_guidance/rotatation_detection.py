...
scale_factors = [.15, .10] 
threshold = 0.70
angles = [0, 90]  # Rotation angles for the template
...
try:
    while True:
        ...
        for scale in scale_factors:
            for angle in angles:
                if angle == 0:
                    ...
                    resized_template = cv2.rotate(cv2.resize(template_image, None, fx=scale, fy=scale), cv2.ROTATE_90_CLOCKWISE)
                ...
                for loc in zip(*locations[::-1]):
                    ...
                    boxes.append([top_left[0], top_left[1], bottom_right[0], bottom_right[1], angle])

        boxes = np.array(boxes)
        indices = cv2.dnn.NMSBoxes(boxes[:, :4].tolist(), [1]*len(boxes), threshold, 0.9)
        ...
        for i in indices:
            ...
            if angle == 0:
                color = (0, 255, 0)  # green for non-rotated template
            else:
                color = (0, 0, 255)  # red for rotated template

            cv2.rectangle(color_image, top_left, bottom_right, color, 2)
            ...
            cv2.putText(color_image, f"Center: ({center_x}, {center_y})", (center_x, center_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
            cv2.putText(color_image, f"Depth: {depth}", (center_x, center_y - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
